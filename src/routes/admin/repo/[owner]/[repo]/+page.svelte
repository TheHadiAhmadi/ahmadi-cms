<script lang="ts">
  ...
  let activeTab = $state<'content' | 'images' | 'pages'>('content');
  ...
  const navGroups = $derived([
    {
      title: 'Content',
      items: [
        { label: 'Collections', href: `/admin/repo/${data.owner}/${data.repo}?tab=content`, icon: Layers },
        { label: 'Images', href: `/admin/repo/${data.owner}/${data.repo}?tab=images`, icon: Image },
      ]
    },
    {
      title: 'Pages',
      items: [
        { label: 'Pages', href: `/admin/repo/${data.owner}/${data.repo}?tab=pages`, icon: FileText },
      ]
    },
    {
      title: 'Settings',
  ...
  function getFileIcon(name: string, type: string) {
    ...
    if (ext === 'js' || ext === 'ts' || ext === 'svelte' || ext === 'vue') return FileCode;
    return File;
  }
  
  function getIconBg(name: string, type: string) {
    if (type === 'dir') return 'bg-amber-500/10 text-amber-600 dark:text-amber-400';
    const ext = name.split('.').pop()?.toLowerCase();
    if (ext === 'md' || ext === 'markdown') return 'bg-blue-500/10 text-blue-600 dark:text-blue-400';
    if (ext === 'json') return 'bg-purple-500/10 text-purple-600 dark:text-purple-400';
    if (ext === 'js' || ext === 'ts') return 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-400';
...
<div class="flex min-h-[calc(100vh-3.5rem)]">
  ...
  <main class="flex-1 p-5">
    <div class="mb-5 flex items-center justify-between">
      <div>
        <h2 class="text-lg font-semibold tracking-tight">Content</h2>
      ...
      <div class="mb-6">
        ...
        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
          {#each data.collections as coll}
            <a
              href="/admin/repo/{data.owner}/{data.repo}/collection/{coll.name}"
              class="flex items-center gap-2 rounded-lg border border-border/50 bg-card p-3 transition-colors hover:bg-secondary/50"
            >
    ...
    <div class="surface w-full max-w-sm p-5 mx-4 scale-in">
      <div class="flex items-center justify-between mb-4">
        ...
        <button 
          ...
          class="flex h-7 w-7 items-center justify-center rounded-md text-[--muted-foreground] transition-colors hover:bg-secondary"
        >
          <X class="h-3.5 w-3.5" />
        </button>
      </div>
      ...
      >
        ...
        <div class="space-y-3">
          <div class="flex gap-1.5 rounded-lg bg-secondary p-1">
            <button
              type="button"
              onclick={() => newFileType = 'file'}
              class="flex-1 rounded-md py-1.5 text-[12px] font-medium transition-all {newFileType === 'file' ? 'bg-card shadow-sm' : 'text-[--muted-foreground]'}"
            >
              File
            </button>
            <button
              type="button"
              onclick={() => newFileType = 'folder'}
              class="flex-1 rounded-md py-1.5 text-[12px] font-medium transition-all {newFileType === 'folder' ? 'bg-card shadow-sm' : 'text-[--muted-foreground]'}"
            >
              Folder
            </button>
          </div>
          ...
          <input
            ...
            placeholder={newFileType === 'folder' ? 'folder-name' : 'filename.md'}
            class="h-9 w-full rounded-lg border border-border/60 bg-transparent px-3 text-[13px] transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/20"
            required
          />
        </div>
        
        <input type="hidden" name="type" value={newFileType} />
        
        <div class="mt-4 flex gap-2">
          <Button variant="secondary" class="flex-1 h-8" onclick={() => showCreateModal = false}>
            Cancel
          </Button>
          <Button class="flex-1 h-8">
            Create
          </Button>
        </div>
      </form>
